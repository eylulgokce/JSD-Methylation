...
run_great <- function(gr, db, txdb = "TxDb.Athaliana.BioMart.plantsmart51"){
  ...
  if (l >= 5) {  # Need at least 5 regions for analysis
...
for(context in contexts) {
  ...
  for(region in unique_classes) {
    ...
    # filter for this genomic region
    ...
    # each temperature separately for this genomic region
    for(temp in temperatures) {
      temp_col <- paste0("JSD_bit_", temp)
      ...
      if(temp_col %in% colnames(region_data)) {
        ...
        # GRanges for this region and temperature
